...
from ..schemas.chat import ChatSessionOut, ChatSessionListItem
...
def mis_sesiones(db: Session = Depends(get_db), usuario: Usuario = Depends(obtener_usuario_actual)):
    return (
        ...
        .filter(ChatSession.usuario_id == usuario.id)
        .order_by(ChatSession.id.desc())
        .all()
    )

@router.get("/sesion/{sesion_id}", response_model=ChatSessionOut)
def sesion_detalle(sesion_id: int, db: Session = Depends(get_db), usuario: Usuario = Depends(obtener_usuario_actual)):
    s = (
        db.query(ChatSession)
...
def admin_sesion_detalle(sesion_id: int, _: Usuario = Depends(verificar_admin), db: Session = Depends(get_db)):
    s = (
        db.query(ChatSession)
        .options(joinedload(ChatSession.mensajes))
